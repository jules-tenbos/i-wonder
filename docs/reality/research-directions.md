...
- **Decoherence as first transition.** Literature: Zurek, Schlosshauer. SPLectrum reading: interaction density producing persistence.
- **Life as self-sustaining interaction density.** Literature: Maturana and Varela (autopoiesis). SPLectrum reading: a being that generates its own language (P0 applied to biology).
- **Abstract thought as decoupled interaction.** Literature: Lakoff, Johnson, Gallagher (embodied cognition). SPLectrum reading: the same relational power expressed in a new medium.
- **Symbolic exchange as interaction freed from presence.** Literature: McLuhan, Ong, Kittler (media theory). SPLectrum reading: each step another expression of P1.
- **AI as horizontal interrelation.** The current threshold. SPLectrum reading through HAICC: a new kind of interactor. Open question: AI describes its internals (brain's perspective), humans describe experience (body's perspective) — different languages for the same structural event.
...
- Relational approaches to time in physics: Rovelli's thermal time hypothesis, Barbour's timeless physics.
- Phenomenological tradition: Husserl's time-consciousness, Heidegger's temporality as care-structure.
- Information-theoretic time: Wheeler's "it from bit," Lloyd's computational universe.
- Connection to Shannon: information as difference against a background. Information capacity grows with interaction capacity.
...
- The expectation of observer-independent reality across domains — its history, motivations, philosophical roots (primarily the Enlightenment project).
- Why the resistance takes the same form in QM, linguistics, biology, and AI — the same demand for a "view from nowhere."
- Whether the resistance constitutes evidence that something structural is being touched.
- Connection to Rorty: the pragmatist critique of the mirror of nature.
...
- If P0 and P1 generate the rest, transitions conform to what P0 does across time — differentiation at increasing density, producing thresholds.
- The Yoneda Lemma as formal grounding for P2 — relevant to the "no outside view" stance.
- Functors as structure-preserving mappings — relevant to P4 and to the transitions.
- The exponential curve as a categorical phenomenon: more objects, more morphisms, more composition.
