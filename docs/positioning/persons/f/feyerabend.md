...
**Against method.** There is no fixed set of rules that governs successful science. Every methodological principle — however reasonable — has been productively violated at some point in the history of science. Galileo succeeded not by following method but by breaking it: using rhetoric, ignoring counter-evidence, reinterpreting observations to fit his theory.

**Epistemological anarchism.** "The only principle that does not inhibit progress is: anything goes" (*Against Method*, ch. 1) — not an endorsement of chaos, but the recognition that every fixed rule would have blocked some actual discovery. Any rule imposed in advance would have prevented some actual discovery. The point is not that there are no standards, but that no standard can be fixed in advance across all contexts.
