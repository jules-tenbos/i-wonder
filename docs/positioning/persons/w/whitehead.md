---
layout: default
title: "Alfred North Whitehead (1861–1947)"
description: "Whitehead built reality from events, not substances — every actual entity constituted by its relations, creativity the universal of universals."
---

[Home](/) > [Positioning](/positioning/) > [Persons](/positioning/persons/) > Whitehead

# Alfred North Whitehead (1861–1947)

Whitehead matters to SPLectrum because he built the most thorough relational ontology in Western philosophy. Nothing exists in isolation — every actual entity is constituted by its relations to every other. That is P1 as metaphysics. His insistence that reality is [process](/positioning/subjects/p/process-philosophy/), not substance — events arising, achieving satisfaction, perishing, taken up by what follows — connects directly to the [seed](/seed/)'s treatment of historicity and growing complexity (P5). And his rejection of the bifurcation of nature — the split between the world as it is and the world as experienced, inherited from [Descartes](/positioning/persons/d/descartes/) — resonates with P2's denial of the outside view. Where [Rorty](/positioning/persons/r/rorty/) dismantled the mirror from the side of language, Whitehead dismantled it from the side of metaphysics.

[Alfred North Whitehead](https://plato.stanford.edu/entries/whitehead/) (1861–1947). Three distinct careers, each producing major work. The first thirty years at Cambridge (Trinity College) as a mathematician — *Principia Mathematica* with Russell, foundational work in logic and algebra. Then London (Imperial College, University of London), where he turned toward philosophy of science, developing a relational theory of space and time as an alternative to Einstein's. At 63 he moved to Harvard, where he built the speculative metaphysical system of *Process and Reality*. Nothing exists in isolation — every actual entity is constituted by its relations to other actual entities. Reality is not made of enduring substances but of events that arise, achieve their satisfaction, and perish, each one taking up what came before and contributing to what comes after.

---

## Key concepts

**Actual occasions.** The basic units of reality — not things but happenings. Each actual occasion arises through *concrescence*: the process of drawing together past occasions, unifying them into a new moment of experience. When concrescence is complete, the occasion becomes a datum for future occasions.

**Prehension.** How one actual occasion takes account of another. A prehension is not perception in the human sense — it is any mode of grasping, incorporating, or being affected by what has already occurred. Positive prehensions include; negative prehensions exclude. Together they shape what the new occasion becomes.

**Creativity.** The ultimate category — "the universal of universals." Creativity is the principle by which the many become one and are increased by one. Every new actual occasion is a creative act: something genuinely new arising from what was given. Not repetition but advance.

**The bifurcation of nature.** Since Galileo and [Descartes](/positioning/persons/d/descartes/), modern science has split nature into two — the nature that is (primary qualities: mass, extension, motion) and the nature that is perceived (secondary qualities: colour, sound, warmth). Whitehead rejects this bifurcation: nature is one, and what we perceive is as real as what physics measures. The split is not in nature but in our theorising.

**The fallacy of misplaced concreteness.** Taking abstractions for concrete realities. When science treats its models as the things themselves — when the map replaces the territory — it commits this fallacy. The lived world is richer than any abstraction drawn from it.

**Process and permanence.** Whitehead holds both together. Each actual occasion perishes, but its achievement is taken up — *objectified* — in subsequent occasions. The past is not lost; it is the material from which the present arises. Permanence is not the opposite of process but its consequence.

**God.** Whitehead's God is not the God of traditional theism. It is the principle that provides the initial aim for each actual occasion (the primordial nature — the ordering of all possibilities) and the preservation of all achieved value (the consequent nature — God as shaped by the world's experience). God is not outside the process; God is the most complete instance of it. Process theology grew from this; secular readers of Whitehead tend to set it aside. The concept remains one of the most contested aspects of his philosophy.

---

## Where Whitehead stops

Whitehead built the most complete relational metaphysics available — but the system is speculative and totalising in a way SPLectrum resists. It aims to describe *everything*: every actual occasion, from an electron to God, fits the same categorical scheme. The ambition is magnificent but also the problem. SPLectrum's seed is not a theory of everything — it is a set of principles about language and relation that can be applied across domains without claiming to exhaust them. Whitehead also stays with the individual occasion as the unit of analysis; the social constitution of shared reality through language (P3) is not his question. And his God — however unconventional — introduces a principle of order that SPLectrum's [equal standing](/seed/) (P4) does not need.

---

## Key works

- [*Principia Mathematica*](https://en.wikipedia.org/wiki/Principia_Mathematica) (1910–13, with Bertrand Russell) — the foundations of mathematics; the Cambridge career
- [*The Concept of Nature*](https://en.wikipedia.org/wiki/The_Concept_of_Nature) (1920) — nature as process, against the bifurcation of nature into appearance and reality
- [*Science and the Modern World*](https://en.wikipedia.org/wiki/Science_and_the_Modern_World) (1925) — the fallacy of misplaced concreteness, the history of scientific abstraction
- [*Process and Reality*](https://en.wikipedia.org/wiki/Process_and_Reality) (1929) — the full metaphysical system: actual occasions, prehension, creativity, God
- [*Adventures of Ideas*](https://en.wikipedia.org/wiki/Adventures_of_Ideas) (1933) — civilisation as the interplay of ideas: beauty, truth, adventure, peace

---

See also: [Process philosophy](/positioning/subjects/p/process-philosophy/) · [Category theory](/positioning/subjects/c/category-theory/) · [The seed and Philosophy](/seed/philosophical/) · [The seed and Interrelational Pluralism](/seed/interrelational-pluralism/)
