---
layout: default
lastmod: 2026-06-06
title: "Language Substrate"
description: "The committed languages of the SPL platform: AVRO for structure, Git for historicity, URI and XPath for addressing, Kafka for mobility."
---

[Home](/) > [Engineering](/engineering/) > [SPL Platform](/engineering/spl/platform/) > Language Substrate

# Language Substrate

The committed languages. Each is a language in its own right — with its own vocabulary, grammar, and idioms — and each plays a specific structural role. Pre-decided commitments, not discoveries of necessity; the fit between architectural requirement and chosen language is what makes each a good commitment.

- **[AVRO](avro)** — language of structure. Schemas as carriers, namespaces as meaning contexts, conformance discovered through reader/writer resolution.
- **[Git](git)** — language of historicity. Commits, refs, DAG, hard boundary, decentralised exchange.
- **[Addressing](addressing)** — URI and XPath. URI identifies which resource; XPath navigates where within it; addresses composed at the point of use.
- **[Kafka](kafka)** — language of mobility. Record envelope, context-in-transit, arrival order and timestamp.
