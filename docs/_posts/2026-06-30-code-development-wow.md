...
Dependencies split into two kinds, and the split matters. Constitutive dependencies are part of the architecture — forked from upstream, adapted for the Bare runtime, maintained locally. They live under `lib/` in the repo as git subtrees, versioned alongside everything else. These are not third-party code that happens to be present. They are as much part of the codebase as any other module. The AVRO type system and its RPC layer sit here — barified, owned, free to diverge when the architecture requires it. Platform dependencies are different. These are the runtime itself — Holepunch's Bare modules. You build on them, you don't own them. They are populated into `lib/` by a setup script, pinned to specific versions, and gitignored — present for development, not committed to the repo. The boundary between the two is permeable: if a platform module needs a fix, it moves to constitutive. If upstream catches up, it can move back.

The npm registry plays no role in the code chain. Constitutive code is already in the repo. Platform code is version-pinned and populated locally. The one remaining touch point is native addon prebuilds — compiled binaries that npm hosts but GitHub repos don't — and that is being eliminated by building native addons from source. The goal is simple: no intermediary between the source tree and what runs. Lock files pin versions but still fetch from the registry — here, the source tree is the lock.
...
<small>This post is part of the [engineering series](/blog/label/engineering).</small>
