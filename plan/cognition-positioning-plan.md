...
First artifact. Doesn't require the axis to be settled.

Route: start from the dictionary entry, notice what "mental" and "acquiring" are doing, arrive at the restrictions lifted. The processor image probably belongs here. Concrete, thinking-in-motion, one point.

### 2. Seed page — the seed and cognition

The load-bearing piece. Carries the **structural side only**.

Content: the gap in P2, then the axis — substrate, engagement, sense-making, with the recursion. The argument for the page existing is the gap, and it is stronger than any argument about the definition.
...
### 3a. Close-affinity piece in the seed ring — the resonance cast (agreed 2026-08-11)

A seed-ring piece (the sharing-knowledge bundle is the structural precedent) gathering the philosophers who move cognition in close — organized by the leg of the position each resonates on:

- **No-two-step** (cognition constitutive of experiencing): Kant (the classical origin — synthesis runs in all perception; caveat: fixed universal categories vs historicity-laden specific processes), Merleau-Ponty (the closest of anyone — the *Phenomenology of Perception* as one long argument against both forms of the two-step), Ryle (the regress against the intellectualist legend), Wittgenstein (understanding is not a mental process; *Abrichtung* as community priming), Peirce (perceptual judgment as extreme abduction).
- **Primed / the belonging leg**: Gadamer (*Vorurteil* and effective history — the socially funded always-already almost verbatim), Husserl (passive synthesis), Bergson (double hit: perception memory-saturated *and* selective — extraction-as-reduction stated 1896), Polanyi (tacit knowing, indwelling).
- **Running-process** (relations already in experience): James (radical empiricism — relations given in the stream, not added after), Dewey (against the spectator theory; knowing as doing within experience), Whitehead (prehension — no substrate experience happens *to*), Hegel (*Erfahrung* as self-transforming).
- **Specific-way**: von Uexküll (Umwelt — each kind's own disclosed world, per its own process).

All fourteen have person pages — fully link-ready. **This piece is the docking question's home**: the Dewey/James-vs-phenomenology open item resolves as both-on-different-legs, written out as the piece's own architecture. Division of labour: the ring piece holds the *resonance*; the subject page holds the *contest of cuts* (the same figure may appear in both). Register per the standing close-affinity discipline: the field is the subject, resonance only, no grading, links down to persons only. **A companion blog post** (conversations register, pluralism-landscape precedent) parks on a 2030 date until a slot opens. Timing: after the in-flight position pieces — no deadline.
...
Organised around **deferred**, not around absence. The territory is the contest: a word every discipline needs and none owns, where the disputes about it are extension disputes wearing definitional clothes. The page holds what cuts into cognition — where each entrant cuts and how far it goes — without settling it.

Candidate material cutting in: cognitivism, enactivism and embodied cognition, predictive processing and the free energy principle, basal cognition, extended and distributed mind, animal signalling, machine learning, institutional and collective sense-making, the deflationary position.

The payoff is assembly: these live in separate literatures with little traffic between them, and the incisions are real — they just don't get read together.

**Watch:** deferred must not slide into lack. Reading the vacancy as a definition-not-yet-achieved colours the field as awaiting resolution and puts SPLectrum in line as the one that finally delivers. The vacancy is a feature of what the word does.

**Relation to the pillar:** the pillar is not the resolution the page defers to. The page holds the cuts as cuts; the pillar is one position among them — structurally distinctive, peer standing on the content side.
...
Position pieces first — none need territory: Post A, then the seed page rework with Post B alongside it (the page live by Post B's publish date). **Reordered 2026-08-11:** then the close-affinity resonance piece and its companion post, with the **cognition subject page started as part of that work** (the pluralism precedent — ring and subject built as a pair: resonance on one side of the fence, the contest of cuts on the other; the subject page as the frame the disciplines phase later delivers into). The third post (the metaphysics context, content side) follows after, unscheduled. **Register note on the subject page:** scoped by our definition, voiced by the field. The scope rule, in Jules's words (2026-08-11): **drop the mental constraint, and allow experience to encompass all activity** — so the page's territory is everything that acquires knowledge and understanding through any activity, on any substrate: basal, distributed, machine, 4E, the human case. The width is also field-warranted via the extension disputes, so the page covers it in outside voice with no SPLectrum vocabulary; the definition's authoritative homes stay seed-side (page, vocabulary, ring). The deferred-not-lack watch applies in full.
...
- Experience has not had the treatment cognition has just had. Whether the axis needs it before it can be built on. *(Largely answered from the site, 2026-08-09: the historicity seed page is the experience station's treatment — retention, accumulation, the temporal span. The axis can point at it and build now; whether experience wants more later is a separate question.)*
- On the philosophical side, experience-as-inflow is narrower than phenomenology's experience; the docking may be tighter with Dewey and James than with the phenomenological line. *(Resolved in principle 2026-08-11: both lines dock, on different legs — phenomenology on no-two-step and priming, pragmatism on running-process and relations-in-experience. Written out as the architecture of the seed-ring resonance piece (§ 3a); the narrower-experience caveat stays live for that piece's drafting.)*
