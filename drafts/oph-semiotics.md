...
- But the formal framework has five axioms that presuppose operator algebras, Hilbert spaces, quantum focusing, recovery channels, the holographic principle. These are not minimal. They are full theoretical positions — conclusions from decades of physics — repackaged as starting points.
- The holographic principle itself comes out of QM applied to gravity. The framework stands on what it claims to derive.
- Foundational axioms work when they are minimal. Two principles — differentiation and relational character — can be formalised by category theory. Everything else follows. That is minimal. Five axioms that presuppose the entire physics stack is not.

### 5. Observers that must agree — no freedom, no semiosis
- The consistency axiom: overlapping observers must agree on shared observables. Agreement is postulated, not produced.
- What grounds observers on the same floor? What produces agreement? What happens when they don't agree? RQM works on exactly this — decoherence as the mechanism. OPH skips it.
- And: observers that must agree are not free to disagree. An observer without that freedom is a node in a consensus protocol, not a genuine perspective.
- In semiosis, interpretation can fail, drift, be contested. Meaning is not guaranteed. That is what makes it a living process. OPH's version has the outcome built in — the axiom guarantees convergence. That is representation, not semiosis.

### 6. Measurement — correct by axiom
- "The measurement problem dissolves. A measurement is the moment when a local pattern is stabilized across observers as a shared record."
- But if observers must agree (axiom), then the shared record is correct by definition. Correct judged against what? There is nothing outside the protocol. The record represents — but what it represents is its own convergence.
- That is representationalism by axiom. The very thing a relational, semiotic framework should avoid.

### 7. TOE and the end of evolution
- OPH presents itself as a Theory of Everything. One framework deriving all of physics.
- If one language is final, every other description is subordinate to it. Equal standing (P4) is eliminated. Languages do not have equal standing if one of them contains all the others.
...
- [ ] Update positioning boundary index if needed
- [ ] Final link check (max 5)
- [ ] Choose scheduling date
