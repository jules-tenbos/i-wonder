...
topic: decentralised cognition, process models, neuroscience
status: storyline
---

# Decentralised Process Models

The seed principles place equal standing in a prominent position. Languages are inter-relational and have equal standing in potential. The common layer — subject, language, reality, entity — inherits that property. A decentralised approach conforms to equal standing with minimal friction. Orchestration introduces hierarchy — something above directing what's below — and that creates friction against a common layer where nothing ranks above anything else. Decentralised is not the only option, but it is the low-friction choice.

A decentralised setting requires appropriate process models, devoid of orchestration. Processes are triggered by data state changes.
...
**Subject creation.** Brain areas differentiate as function demands it. A new area doesn't get designed — it emerges when interaction density in a region reaches sufficient complexity to warrant specialisation. The area becomes its own local reality, with its own processes, its own data state. A new repository spawns when the work demands its own space.
...
- Engineering post with neuroscience depth
- Not "here's our clever process model" but "equal standing has consequences — this is what falls out"
- The deduction is the news: equal standing → no hierarchy → no orchestration → data-state-driven → evolutionary pattern
- Neuroscience as inspiration, not as proof
- Light touch on the brain science — enough to show the parallel, not a neuroscience lecture

---

## Tasks on scheduling

- [ ] Review neuroscience claims for accuracy
- [ ] Consider second example beyond water
- [ ] Update reference page: `docs/engineering/process-models.md`
- [ ] Draft tweet
