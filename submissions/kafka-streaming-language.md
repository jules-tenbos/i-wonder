---
title: "Kafka — The Streaming Language"
type: substantial
status: in-progress
destinations: engineering
---

# Kafka — The Streaming Language

Substantial submission. Kafka as Splectrum's committed language for data mobility. The relationship between logical types and Kafka records. The structural requirements for data in motion and how Kafka fulfils them. The three committed languages and their meeting point in mycelium.

---

## Three Committed Languages

Mycelium operates at the intersection of three mature, committed languages. Each addresses one orthogonal concern:

**AVRO — structure.** The language of data shape. Schema, serialization, conformance. How data describes itself and how "can I read it as" gets asked.

**Git — historicity.** The language of history. Immutability, branching, commits, the memory gradient. How data remembers and forgets.

**Kafka — mobility.** The language of streaming data. Records, topics, partitions, streaming. How data travels.

None of these is mycelium. Mycelium is where they meet — the fabric, the tree structure, the context model, the protocols. The relational space in which three equal-standing languages interoperate.

These are commitments, not discoveries of necessity. The necessity is at the level of what capabilities are needed — self-describing shape, history with forgetting, streaming with context. The choice of which language fulfils each is Splectrum's commitment. The fit between requirement and commitment is what makes it a good commitment, not what makes it a law. Another framework could commit to different languages for the same structural concerns. P4 — equal standing in potential.
...
This is not Kafka's design choice. These are the structural requirements for data leaving the tree and travelling self-sufficiently. Identity, context carriage, arrival order, historicity, self-description. That the Kafka record fulfils all five is why it is the right commitment for streaming data in Splectrum.
...
**Interpretation** — `splectrum.natural.heidegger` — pure reading context. The payload is what it is. The logical type tells you which language to read it in. You must interpret the data payload as Heidegger speak. Retrospective.
...
The challenge: can you think of a logical type that would not make sense as a Kafka record?

`decimal` — value is bytes, headers carry precision and scale, logical type names the encoding transformation. Natural fit.

`date` — value is an int, logical type names the "days since epoch" interpretation, which is also the transformation that produced the int. Natural fit.

Splectrum meaning languages — value is text, logical type names the language that produced it, headers carry concept vocabulary. Natural fit.

Action types — value carries the operation payload, logical type names the intent, headers carry arguments. Natural fit.

A bare physical type without any logical type — data that hasn't entered any language game. This wouldn't be in the fabric, because being in the fabric already means being in context.
...
Records contain realities that contain records. Same pattern, no privileged level. The recursive self-similarity of the persona model expressed in the streaming language.
...
*This document establishes Kafka as Splectrum's committed language for data mobility — alongside AVRO for structure and git for historicity. The Kafka record is the necessary form for data in streaming context, fulfilling structural requirements that any extraction into motion must meet. Mycelium is where the three committed languages meet.*
